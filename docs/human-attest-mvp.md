...
| Attacker attaches their own authenticator to a stranger's principal, using only a leaked or guessed `principal_id` | A single-use, principal-bound, expiring (15-minute) enrolment token — returned once, at principal creation, and never echoed anywhere else — gates both enrolment endpoints. `principal_id` alone proves nothing: it is not secret, since this same service embeds it in `approve_url`'s query string for the separate, later approval step. (This narrows the trust requirement rather than eliminating it: it now rests on the enrolment link and its token reaching the right human over an out-of-band channel — email, Slack, or similar — that this prototype has no visibility into and cannot itself secure. A token intercepted in transit is exactly as good to an attacker as a `principal_id` used to be.) |
